        ...
        if epoch % log_every == 0:
            ...
            for key, val in loss[1].items():
                if key == "props":
                    for prop_num, prop_val in enumerate(val):
                        self.write_data(
                            f"property_{prop_num}", prop_val.item()
                        )
                elif key == "dprops":
                    for prop_num, prop_val in enumerate(val):
                        self.write_data(
                            f"dproperty_{prop_num}", prop_val.item()
                        )
                else:
                    self.write_data(key, val.item())
...
class EnsembleHistoryWriter(BaseHistoryWriter):
    ...
    def __init__(
        self, base_name: str, n_pinns: int, log_every: int, write_every: int
    ) -> None:
        super().__init__(log_every, write_every)
        self.history_writers = [
            HistoryWriter(f"{base_name}_{n}.csv", log_every, write_every)
            for n in range(n_pinns)
        ]
        ...
        for key, val in loss[1].items():
            for n, writer in enumerate(self.history_writers):
                if key == "props":
                    # for prop_num, prop_val in enumerate(val):
                    #   writer.write_data(
                    #       f'property_{prop_num}', prop_val[n].item()
                    #   )
