...
class Trainer:
    def __init__(
        ...
        self.domain = domain
        self.opt = opt
        self.checkpoint_base = Path(
            os.path.join(workdir, "checkpoint", checkpoint_base)
        )
        self.serialise_every = serialise_every
        self.history = HistoryWriter(
            Path(os.path.join(workdir, "history", history_file)),
            log_every, log_every
        )
        self.logger = Logger(
            Path(os.path.join(workdir, "log", log_file)), log_every
        )
        self.output_every = output_every
        self.output_file_base = Path(
            os.path.join(workdir, "results", output_file_base)
        )
