...
class NonAllocatedFunctionSpace(eqx.Module):
    ...
    def compute_field_gradient(self, u, X):
        ...
        return jax.vmap(lambda u, grad_N: u.T @ grad_N, in_axes=(None, 0))(
            u, grad_Ns
        )
    ...
    def integrate_on_elements(self, U, X, state, dt, props, func):
        el_vals = jax.vmap(
            self.integrate_on_element, in_axes=(0, 0, 0, None, None, None)
        )(U, X, state, dt, props, func)
        return np.sum(el_vals)

    def JxWs(self, X: Float[Array, "nn nd"]) -> Float[Array, "nq"]:
        Js = jax.vmap(lambda x, dN: (x.T @ dN).T, in_axes=(None, 0))(
            X, self.shape_functions.gradients
        )
        return jax.vmap(lambda J, w: np.linalg.det(J) * w, in_axes=(0, 0))(
    ...
    def shape_function_gradients(
        self, X: Float[Array, "nn nd"]
    ) -> Float[Array, "nq nnpe nd"]:
        Js = jax.vmap(lambda x, dN: (x.T @ dN).T, in_axes=(None, 0))(
            X, self.shape_functions.gradients
        )
        Jinvs = jax.vmap(lambda J: np.linalg.inv(J), in_axes=(0,))(Js)
        return jax.vmap(lambda Jinv, dN: (Jinv @ dN.T).T, in_axes=(0, 0))(
            Jinvs, self.shape_functions.gradients
        )
...
def construct_function_space(mesh, quadratureRule, mode2D="cartesian"):
    ...
    :param quadratureRule: The quadrature rule to be used for integrating \
        on the domain.
    ...
    with Timer("construct_function_space"):
        shapeOnRef = mesh.parentElement.compute_shapes(
            mesh.parentElement.coordinates, quadratureRule.xigauss
        )
        return construct_function_space_from_parent_element(
            mesh, shapeOnRef, quadratureRule, mode2D
        )


def construct_function_space_from_parent_element(
    mesh, shapeOnRef, quadratureRule, mode2D="cartesian"
):
    """
    Construct a function space with precomputed shape
    function data on the parent element.
    ...
    ----------
    :param mesh: The mesh of the domain.
    :param shapeOnRef: A tuple of the shape function values \
        and gradients on the
    ...
    :param quadratureRule: The quadrature rule to be used for \
    ...
    shapes = jax.vmap(lambda elConns, elShape: elShape, (0, None))(
        mesh.conns, shapeOnRef.values
    )
    ...
    vols = jax.vmap(el_vols, (None, 0, None, None, None, None))(
        ...
        mesh.parentElement,
        shapeOnRef.values,
        shapeOnRef.gradients,
        quadratureRule.wgauss,
    )

    return FunctionSpace(
        shapes, vols, shapeGrads, mesh.conns, quadratureRule, isAxisymmetric
    )


def map_element_shape_grads(
    coordField, nodeOrdinals,
    parentElement, shapeGradients
):
    ...
    Jinvs = jax.vmap(lambda J: np.linalg.inv(J), in_axes=(0,))(Js)
    return jax.vmap(lambda Jinv, dN: (Jinv @ dN.T).T, in_axes=(0, 0))(
        Jinvs, shapeGradients
    )


def compute_element_volumes(
    coordField, nodeOrdinals, parentElement, shapes, shapeGradients, weights
):
    Xn = coordField.take(nodeOrdinals, 0)
    Js = jax.vmap(lambda x, dN: (x.T @ dN).T, in_axes=(None, 0))(
        Xn, shapeGradients
...
def compute_element_volumes_axisymmetric(
    coordField, nodeOrdinals, parentElement, shapes, shapeGradients, weights
):
    vols = compute_element_volumes(
        coordField, nodeOrdinals, parentElement, shapes, weights
    )
    Xn = coordField.take(nodeOrdinals, 0)
    Rs = shapes @ Xn[:, 0]
...
def default_modify_element_gradient(
    elemGrads, elemShapes, elemVols, elemNodalDisps, elemNodalCoords
):
    return elemGrads
...
def interpolate_to_points(functionSpace, nodalField):
    return jax.vmap(interpolate_to_element_points, (None, 0, 0))(
        nodalField, functionSpace.shapes, functionSpace.conns
    )
...
def integrate_over_block(
    ...
    *params,
    ...
    :param func: Lagrangian density function to integrate, Must have the
        signature
        ``func(u, dudx, q, x, *params) -> scalar``, where ``u`` is the
        primal field, ``q`` is the
        value of the internal variables, ``x`` is the current point
        coordinates, and ``*params`` is
        a variadic set of additional parameters, which correspond
        to the ``*params`` argument.
        block: Group of elements to integrate over. This is an
        array of element indices. For
        performance, the elements within the block
        should be numbered consecutively.
    :param modify_element_gradient: Optional function that modifies
        the gradient at the element level.
        This can be to set the particular 2D mode, and additionally to
        enforce volume averaging
        on the gradient operator. This is a keyword-only argument.

    Returns
    A scalar value for the integral of the density functional
    ``func`` integrated over the
    block of elements.
    """
    # below breaks the sphinx doc stuff
    # :param *params: Optional parameter fields to pass into
    # Lagrangian density function. These are

    vals = evaluate_on_block(
        functionSpace,
        U,
        X,
        stateVars,
...
def evaluate_on_block(
    ...
    *params,
    ...
    """Evaluates a density function at every quadrature
    point in a block of the mesh.
    ...
    :param stateVars: Internal state variable array.
    :param dt: Current time increment
    :param func: Lagrangian density function to evaluate,
        Must have the signature
        ```func(u, dudx, q, x, *params) -> scalar```, where ```u```
        is the primal field, ```q``` is the
        value of the internal variables, ```x``` is the current
        point coordinates, and ```*params``` is
        a variadic set of additional parameters, which correspond
        to the ```*params``` argument.
    :param block: Group of elements to evaluate over.
        This is an array of element indices. For
        performance, the elements within the block
        should be numbered consecutively.

    :param modify_element_gradient: Optional function that
        modifies the gradient at the element level.
        This can be to set the particular 2D mode,
        and additionally to enforce volume averaging
        on the gradient operator. This is a keyword-only argument.

    Returns
    An array of shape (numElements, numQuadPtsPerElement)
    that contains the scalar values of the
    ...
    fs = functionSpace
    compute_elem_values = jax.vmap(
        evaluate_on_element,
        (None, None, 0, None, None, 0, 0, 0, 0, None, None,
            *tuple(0 for p in params)),
    )

    blockValues = compute_elem_values(
        U,
        X,
        stateVars[block],
        props,
...
def integrate_element_from_local_field(
    ...
    """
    elemVals = jax.vmap(interpolate_to_point, (None, 0))(
        elemNodalField, elemShapes
    )
    elemGrads = jax.vmap(compute_quadrature_point_field_gradient, (None, 0))(
        elemNodalField, elemShapeGrads
    )
    elemGrads = modify_element_gradient(
        elemGrads, elemShapes, elemVols, elemNodalField, elemNodalCoords
    )
    elemPoints = jax.vmap(interpolate_to_point, (None, 0))(
        elemNodalCoords, elemShapes
    )
    fVals = jax.vmap(func, (0, 0, 0, 0, None))(
        elemVals, elemGrads, elemStates, elemPoints, dt
    )
    return np.dot(fVals, elemVols)


def compute_element_field_gradient(
    U,
    coords,
    elemShapes,
    elemShapeGrads,
...
def integrate_element(
    ...
    elemGrads = compute_element_field_gradient(
        U,
        coords,
        elemShapes,
        elemShapeGrads,
...
def evaluate_on_element(
    ...
    *params
    ...
    fVals = jax.vmap(kernelFunc, vmapArgs)(
        elemVals, elemGrads, elemStates, props, elemXs, dt, *params
    )
    return fVals


def project_quadrature_field_to_element_field(functionSpace, quadField):
    return jax.vmap(average_quadrature_field_over_element)(
        quadField, functionSpace.vols
    )
...
def get_nodal_values_on_edge(functionSpace, nodalField, edge):
    ...
    :param nodalField: The nodal vector defined
        over the mesh (shape is number of
        nodes by number of field components)
    :param edge: tuple containing the element number
        containing the edge and the
...
def interpolate_nodal_field_on_edge(
    functionSpace, U,
    interpolationPoints, edge
):
    """
    ...
    :param interpolationPoints: coordinates of points
        (in the 1D parametric space) to
        interpolate to
    :param edge: tuple containing the element number
        containing the edge and the
        permutation (0, 1, or 2) of the edge within the triangle
...
def integrate_function_on_edge(functionSpace, func, U, quadRule, edge):
    uq = interpolate_nodal_field_on_edge(
        functionSpace, U, quadRule.xigauss, edge
    )
    Xq = interpolate_nodal_field_on_edge(
        functionSpace, functionSpace.mesh.coords, quadRule.xigauss, edge
    )
...
def integrate_function_on_edges(functionSpace, func, U, quadRule, edges):
    integrate_on_edges = jax.vmap(
        integrate_function_on_edge, (None, None, None, None, 0)
    )
