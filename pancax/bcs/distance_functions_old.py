...
def distance(x1, x2):
    return jnp.sqrt(
        jnp.power(x2[0] - x1[0], 2.0) +
        jnp.power(x2[1] - x1[1], 2.0)
    )
...
def line_segment(x, segment):
    ...
    f = (1.0 / L) * (
        (x[0] - x1[0]) * (x2[1] - x1[1]) - (x[1] - x1[1]) * (x2[0] - x1[0])
    )
    t = (1.0 / L) * (
        jnp.power(L / 2.0, 2.0) -
        jnp.power(distance(x, x_c), 2.0)
    )
    varphi = jnp.sqrt(jnp.power(t, 2.0) + jnp.power(f, 4.0))
    phi = jnp.sqrt(
        jnp.power(f, 2.0) + (1.0 / 4.0) *
        jnp.power(varphi - t, 2.0)
    )
    return phi
...
def distance_function(domain, ssets, m=1.0):
    ...
    def inner_func(x):
        ...
        Rs = vmap(lambda x: 1.0 / jnp.power(x, m))(Rs)
        return 1.0 / jnp.power(jnp.sum(Rs), 1.0 / m)
