...
def plot_force_time(fileName, fileName2, linestyle='-'):
    histories = np.load(fileName)
    force = histories['forces']
    # time = histories['time']
    histories = np.load(fileName2)
    time = histories['time']
    plt.plot(time, force, linestyle=linestyle)
...
plt.figure()
# forceTimeFile = 'force_time' + fileType
plot_force_time(forceDispFile, energyFile, '-')
