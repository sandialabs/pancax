...
def swanson_1():
    ...
    return Swanson(
        bulk_modulus=K, A1=A1, P1=P1, B1=B1, Q1=Q1, C1=C1, R1=R1,
        cutoff_strain=0.01
    )
...
def simple_shear_test(model):
    ...
    dt = 1.0
    gammas = jnp.linspace(0.05, 1.0, 100)
    Fs = jax.vmap(simple_shear)(gammas)
    grad_us = jax.vmap(lambda F: F - jnp.eye(3))(Fs)
    B_bars = jax.vmap(
        lambda F: jnp.power(jnp.linalg.det(F), -2.0 / 3.0) * F @ F.T
    )(Fs)
    ...
    def vmap_func(B_bar, I1_bar, J):
        psi_an = (
            K * (J * jnp.log(J) - J + 1.0)
            + 1.5 * A1 / (P1 + 1.0) * (I1_bar / 3.0 - 1.0) ** (P1 + 1.0)
            + 1.5 * C1 / (R1 + 1.0) * (I1_bar / 3.0 - 1.0) ** (R1 + 1.0)
        )
        dUdI1_bar = (
            0.5 * A1 * (I1_bar / 3.0 - 1.0) ** P1
            + 0.5 * C1 * (I1_bar / 3.0 - 1.0) ** R1
        )
        B_bar_dev = B_bar - (1.0 / 3.0) * jnp.trace(B_bar) * jnp.eye(3)
        sigma_an = (2.0 / J) * dUdI1_bar * B_bar_dev + \
            K * jnp.log(J) * jnp.eye(3)
...
def uniaxial_strain_test(model):
    ...
    state_old = jnp.zeros((100, 0))
    dt = 1.0
    lambdas = jnp.linspace(1.2, 4.0, 100)
