...
# from pancax import Swanson6, Swanson6FixedBulkModulus
# from .utils import *
# import jax
...
# @pytest.fixture
# def swanson():
#     return Swanson6(cutoff_strain=0.01)


# @pytest.fixture
# def swanson_fbm():
#     return Swanson6FixedBulkModulus(bulk_modulus=10.0, cutoff_strain=0.01)


# @pytest.fixture
# def properties():
#     return FixedProperties([10.0, 0.93074321, -0.07673672, 0.0, 0.5, 0.10448312, 1.71691036])


# @pytest.fixture
# def properties_fbm():
#     return FixedProperties([0.93074321, -0.07673672, 0.0, 0.5, 0.10448312, 1.71691036])
...
#     I2_bars = jax.vmap(model.I2_bar)(Fs)
#     psis = jax.vmap(model.energy, in_axes=(0, None))(Fs, props())
#     sigmas = jax.vmap(model.cauchy_stress, in_axes=(0, None))(Fs, props())

#     K, A1, P1, B1, Q1, C1, R1 = model.unpack_properties(props())

#     for (psi, sigma, I1_bar, I2_bar, J, B_bar) in zip(psis, sigmas, I1_bars, I2_bars, Js, B_bars):
#         psi_an = K * (J * jnp.log(J) - J + 1.) + \
#                  1.5 * A1 / (P1 + 1.) * (I1_bar / 3. - 1.)**(P1 + 1.) + \
#                  1.5 * B1 / (Q1 + 1.) * (I2_bar / 3. - 1.)**(R1 + 1.) + \
#                  1.5 * C1 / (R1 + 1.) * (I1_bar / 3. - 1.)**(R1 + 1.)
#         dUdI1_bar = 0.5 * A1 * (I1_bar / 3. - 1.)**P1 + \
#                     0.5 * C1 * (I1_bar / 3. - 1.)**R1
#         dUdI2_bar = 0.5 * B1 * (I2_bar / 3. - 1.)**Q1
#         B_bar_dev = B_bar - (1. / 3.) * jnp.trace(B_bar) * jnp.eye(3)
#         sigma_an = (2. / J) * (dUdI1_bar + I1_bar * dUdI2_bar) * B_bar_dev + K * jnp.log(J) * jnp.eye(3)
...
#     assert props[3] == 0.0
#     assert props[4] == 0.5
#     assert props[5] == 0.10448312
#     assert props[6] == 1.71691036
