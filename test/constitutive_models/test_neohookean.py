...
def neohookean_1():
    ...
    return NeoHookean(bulk_modulus=K, shear_modulus=G)
...
def simple_shear_test(model):
    ...
    def vmap_func(gamma, I1_bar, J):
        psi_an = 0.5 * K * (0.5 * (J**2 - 1) - jnp.log(J)) +\
            0.5 * G * (I1_bar - 3.0)
        sigma_11_an = 2.0 / 3.0 * G * gamma**2
        sigma_22_an = -1.0 / 3.0 * G * gamma**2
        sigma_12_an = G * gamma
...
def uniaxial_strain_test(model):
    ...
    state_old = jnp.zeros((100, 0))
    dt = 1.0
    lambdas = jnp.linspace(1.0, 4.0, 100)
    ...
    def vmap_func(lambda_, I1_bar, J):
        psi_an = 0.5 * K * (0.5 * (J**2 - 1) - jnp.log(J)) +\
            0.5 * G * (I1_bar - 3.0)
        sigma_11_an = 0.5 * K * (lambda_ - 1.0 / lambda_) + 2.0 / 3.0 * G * (
            lambda_**2 - 1.0
        ) * lambda_ ** (-5.0 / 3.0)
        sigma_22_an = 0.5 * K * (lambda_ - 1.0 / lambda_) - 1.0 / 3.0 * G * (
            lambda_**2 - 1.0
        ) * lambda_ ** (-5.0 / 3.0)
