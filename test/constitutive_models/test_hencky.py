...
def hencky_1():
    ...
    return Hencky(bulk_modulus=K, shear_modulus=G)
...
def simple_shear_test(model):
    ...
    jax.vmap(
        lambda trE, devE, J: (K * trE * jnp.eye(3) + 2.0 * G * devE) / J,
        in_axes=(0, 0, 0),
    )(trEs, Edevs, Js)
    ...
    # assert False
    # for (psi, sigma, gamma, I1_bar, J) in \
    # zip(psis, sigmas, gammas, I1_bars, Js):
