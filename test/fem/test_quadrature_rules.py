...
def quadratures_fix():
    ...
    from pancax.fem.elements import Quad4Element, Quad9Element
    from pancax.fem.elements import SimplexTriElement
    from pancax.fem.elements import Tet4Element, Tet10Element
    ...
    return elements_to_test, in_domain_methods, q_degrees


def test_are_postive_weights(quadratures_fix):
    from pancax.fem import QuadratureRule
    from pancax.fem.elements import Tet4Element, Tet10Element
    import jax.numpy as jnp

    els, _, qrs = quadratures_fix
    for el, q in zip(els, qrs):
        if type(el) is Tet4Element and q == 2:
            pytest.skip("Not relevant for Tet4Element and q_degree = 2")
        if type(el) is Tet10Element and q == 2:
...
def test_are_inside_domain(quadratures_fix):
    from pancax.fem import QuadratureRule
    els, is_insides, qrs = quadratures_fix
    for el, q, is_inside in zip(els, qrs, is_insides):
        qr = QuadratureRule(el, q)
...
def test_error_raise_on_bad_quadrature_degree():
    ...
    with pytest.raises(ValueError):
        QuadratureRule(Hex8Element(), 3)

    with pytest.raises(ValueError):
        QuadratureRule(Quad4Element(), 4)

    with pytest.raises(ValueError):
        QuadratureRule(SimplexTriElement(1), 11)

    with pytest.raises(ValueError):
        QuadratureRule(Tet4Element(), 3)
