...
def create_mesh_and_disp(
    ...
    nodeSets["left" + setNamePostFix] = \
        jnp.flatnonzero(coords[:, 0] < xRange[0] + tol)
    nodeSets["bottom" + setNamePostFix] = jnp.flatnonzero(
        coords[:, 1] < yRange[0] + tol
    )
    nodeSets["right" + setNamePostFix] = \
        jnp.flatnonzero(coords[:, 0] > xRange[1] - tol)
    nodeSets["top" + setNamePostFix] = \
        jnp.flatnonzero(coords[:, 1] > yRange[1] - tol)
    ...
    sideSets = {}
    sideSets["left" + setNamePostFix] = \
        create_edges(coords, conns, is_edge_on_left)
    sideSets["bottom" + setNamePostFix] = \
        create_edges(coords, conns, is_edge_on_bottom)
    sideSets["right" + setNamePostFix] = \
        create_edges(coords, conns, is_edge_on_right)
    sideSets["top" + setNamePostFix] = \
        create_edges(coords, conns, is_edge_on_top)
    ...
    mesh = construct_mesh_from_basic_data(
        coords, conns, blocks, nodeSets, sideSets
    )
    print(mesh)
